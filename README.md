...
홈/상태선택 화면 상단의 "목표 설정하기"를 통해 선택적으로 목표 문구(예: "학점 4.5 달성")와 하루 목표 시간(시간 단위)을 입력할 수 있습니다. 목표가 있으면 "학점 4.5 달성을 위해 시작하자" 같은 동기부여 문구가, 하루 목표 시간이 있으면 자정 기준으로 리셋되는 "오늘 목표 10시간 중 25% 채웠어요" 진행률이 노출되며, 완료/종료 화면의 "오늘 진행한 시간"에도 같은 퍼센트가 함께 표시됩니다. 진행률은 텍스트뿐 아니라 홈/상태선택 화면 상단의 작은 게이지 바로도 시각화됩니다.

이 데이터는 개인 목표라는 민감한 정보일 수 있어 **`localStorage`(`task_reentry_goal_v1`)에만 저장되고 Supabase로는 전송되지 않습니다.** 기기를 바꾸면 초기화됩니다. 5초 내 즉시 이탈로 판정된 세션([즉시 이탈 처리](#즉시-이탈quick-exit-처리) 참고)은 오늘 누적 시간에도 반영되지 않아 지표가 일관됩니다.
